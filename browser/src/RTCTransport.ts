/**
 * Browser RTCTransport — host-side WebRTC Transport for browser-as-host (p2p.md §12 Phase 3).
 *
 * Reuses the §4.5 shared negotiation core (HostNegotiator) with native browser adapters
 * (RTCPeerConnection / RTCDataChannel — no new runtime dependency). Made possible by the
 * §3.3 isomorphic core split: @rivalis/core's default entry has no node-only deps, so a
 * browser bundler can import Transport / TLayer / KickReason directly.
 *
 * Satisfies the same five-step Transport seam as @rivalis/node RTCTransport:
 ...
 *
 * Trust note (p2p.md §8): a browser host is only as trustworthy as the client
 * running it. Browser-host suits casual/co-op games; competitive/authoritative
 * games should use a Node host (@rivalis/node RTCTransport) instead.
 *
 * Frame-size handling (p2p.md §7): frames larger than RTC_MAX_FRAME_BYTES (16 KiB)
 * are automatically chunked before sending and reassembled before delivery using
 * the reserved __rivalis:chunk topic. Oversized frames (>255 chunks, >~4 MiB) are
 * logged as warnings and dropped — never silently truncated.
 */
...
const signalCodec = createCodec({
    namespace: '@rivalis/browser/rtc-transport',
    major: 1,
    schema: {
        Welcome: [
...
export type RTCTransportOptions = {
     ...
     * dropped. Default: 1 MiB (same as WSTransport).
     */
    ...
    private onChannelOpen(channel: RTCDataChannelLike, peerId: string): void {
        ...
        channel.onMessage((buf) => {
            ...
            void (async () => {
                ...
                // pendingEmits buffer (TLayer.ts:58-66, cap 256) drains right now.
                layer.on('message', aid, (_id, m) => {
                    ...
                    if (m.byteLength <= RTC_MAX_FRAME_BYTES) {
                        try { channel.sendBinary(m) } catch { /* channel closed between check and send */ }
                        return
                    }
