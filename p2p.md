...
- [x] **D4** Node WebRTC lib: `node-datachannel` default, `werift` dev/CI fallback (`RIVALIS_WEBRTC_BACKEND=werift`). (§4.5, §13.4) — decided 2026-06-09; rationale in `node/CHANGELOG.md`. **Amended 2026-06-24:** the `werift` fallback was removed — it transitively pulled `ip` (GHSA-2p57-rm9w-gvfp, SSRF, no upstream fix) and `uuid` (GHSA-w5hq-g745-h8pq), which were the only `npm audit` findings in the repo. `node-datachannel` is now the sole backend; the `createPeerConnection` factory keeps the layer backend-agnostic. See `node/CHANGELOG.md`.
...
- [x] `@rivalis/browser` adds `@rivalis/core` kernel dep for `Client`/types + new exports. (§9) — `peerDependencies["@rivalis/core"]` updated to `>=7 <8`, `devDependencies["@rivalis/core"]` added `"*"` for workspace resolution (mirrors fleet/node pattern); `RTCClient` already exported from `browser/src/main.ts` (task 069); no native WebRTC runtime dep added (native browser APIs used); task 070, 2026-06-09.
...
- [x] `demo/src/p2p/`: unchanged `TttRoom` + Node host w/ `RTCTransport` + existing React client on `RTCClient` (parallels `demo/src/fleet/`). (§11) — `demo/src/p2p/index.ts`: signal server (bare Rivalis + WSTransport + `DemoP2PSignalAuth` + `SignalRoom`) on port 9000 co-located with game host (Rivalis + `RTCTransport` + `ArenaAuthMiddleware` + `TttRoom`); `demo/src/p2p/useRoomRTC.ts`: drop-in React hook (`useRoomRTC`) replacing `useRoom` with `RTCClient` pointed at the signal server; `demo/src/p2p/protocol.ts`: shared constants (`SIGNAL_PORT=9000`, `SIGNAL_ROOM_ID`, `HOST_SIGNAL_TICKET`); `npm run p2p -w @rivalis/demo`; task 075, 2026-06-09. **Divergence from §11 plan:** The signal server uses a demo-specific `DemoP2PSignalAuth` rather than `SignalAuthMiddleware` because `RTCClient.connect(ticket)` forwards the SAME ticket to both the signal server and the game host (§4.2 first-binary-message protocol). Since the game ticket format is `<roomId>|<name>|<color>` but `SignalAuthMiddleware` expects `<roomId>:<secret>`, a custom auth extracts only the `roomId` from the game ticket for signal-room routing, delegating full validation to `ArenaAuthMiddleware` on the game host. This is demo-intentional; a production deployment would use separate tickets or a dedicated signal auth strategy.
- [x] Playwright/headless-Chromium two-tab test vs Node host (play move, both boards update). (§10) — `demo/test/p2p-browser.test.ts`: two browser contexts, each navigating to `demo/src/p2p/test-client.html` (served by Vite via `demo/vite.p2p-test.config.ts` on port 5174); test waits for both tabs to reach `status='playing'`, player X places at index 0, asserts `board[0]==='X'` on both tabs; skips gracefully when `node-datachannel` native binary is absent; webServers managed by `demo/playwright.config.ts` (P2P server on port 9000 via `ts-node src/p2p/index.ts`, Vite on port 5174); `npm run test:e2e -w @rivalis/demo`; task 076, 2026-06-09.
...
- [x] Optional `Room.serialize()/hydrate()` for host-handoff state transfer. (§12 phase 3) — `Room` gains two protected hooks: `serialize(): Uint8Array | null` (default `null` = opt-out) and `hydrate(bytes: Uint8Array): void` (default no-op). Framework-internal `trySerialize()`/`tryHydrate()` wrappers swallow user throws to keep the handoff path crash-safe. Transfer mechanism: the outgoing host sends its serialized state to the signal server via a new `signal:host_state` wire message (binary `HostState` type, `F.BYTES` field, appended APPEND-ONLY to `signal/src/wire/index.ts` schema at tag 2); `SignalRoom` buffers the bytes in `pendingHostState` (guarded: only accepted from the current `hostId`). On election `SignalRoom` delivers the buffered state directly to the newly-elected host via a targeted `signal:host_state` send (not broadcast), then clears `pendingHostState` so stale state from a previous epoch cannot reach a second election. For unintentional disconnects (crash) no state is available and the new host starts fresh — non-implementing rooms are entirely unaffected. Tests: `core/test/room-serialize-hydrate.test.mts` (hooks + error resilience); `signal/test/signal-room.test.mts` + `signal/test/wire.test.mts` (wire round-trip + handoff flow); task 083, 2026-06-09.
...
- [x] Pure-JS STUN dev-only responder behind a flag. (§4.3, §12) — `DevStunResponder` class in `signal/src/DevStunResponder.ts`; UDP/IPv4, handles STUN Binding Requests only (RFC 8489 §6.3); XOR-MAPPED-ADDRESS response; no TURN relay; enabled via `RIVALIS_STUN_DEV=true` env var (+ optional `RIVALIS_STUN_DEV_PORT`/`RIVALIS_STUN_DEV_HOST`) or via `SignalServerOptions.stunDev`; disabled by default; exported from `@rivalis/signal`; `SignalServer` auto-starts the responder when the env flag is set; tests in `signal/test/dev-stun-responder.test.mts`; task 087, 2026-06-09.
- [~] `werift` dev/CI fallback behind a flag (no native build). (§4.5, §12) — **REMOVED 2026-06-24** (security: werift → `ip` SSRF GHSA-2p57-rm9w-gvfp, no upstream fix; cleared all 4 `npm audit` findings — see `node/CHANGELOG.md`). Historical record of the implementation below. — `node/src/peer/RTCPeer.ts`; `WeriftPeer` (full adapter, replaces the Phase 1 stub) and `WeriftDataChannel` implement `RTCPeerLike`/`RTCDataChannelLike` over werift's async API using an internal promise queue to serialise `createOffer`/`createAnswer`/`setLocalDescription`/`setRemoteDescription`/`addIceCandidate`; `onicecandidate` maps to `onLocalCandidate`; `onconnectionstatechange` maps to `onStateChange`; `ondatachannel` maps to `onDataChannel`; selected by `RIVALIS_WEBRTC_BACKEND=werift`; werift is an `optionalDependency` (`npm install --omit=optional` skips it); loopback test passes in `node/test/rtc-werift-loopback.test.mts`; `node/test/rtc-peer-adapter.test.mts` factory test updated to handle both installed and not-installed werift; task 088, 2026-06-09. **Behavioural note vs §4.5 plan:** werift's RTCPeerConnection API is async (all SDP and ICE operations return Promises), unlike node-datachannel's synchronous-callback design. The adapter bridges this with an internal operation queue (`_enqueue`) so that synchronous back-to-back calls from `NegotiationCore` (e.g. `setRemoteDescription(offer)` immediately followed by `setLocalDescription('answer')`) are serialised correctly without requiring changes to the negotiation state machine.
...
- [x] Game logic (`Room`/`Actor`): **zero changes** — assert across all phases. (§5, §11) — `core/test/game-logic-zero-transport-imports.test.mts`: reads every Room subclass in `demo/src/server/` (all files except the server bootstrap `index.ts`) and asserts zero transport-specific imports; forbidden set covers `ws`, `node-datachannel`, `@rivalis/node`, `@rivalis/browser`, `@rivalis/signal`, and the WS subpath exports from core; per-file tests name the offending import as a design regression; summary test aggregates all violations. Positive-case evidence already present: `demo/src/p2p/index.ts` imports `TttRoom` directly from `demo/src/server/TttRoom.ts` unchanged for the RTCTransport host (same file used by the WSTransport server `demo/src/server/index.ts`); `node/test/rtc-loopback.test.mts` verifies the identical game logic works end-to-end over real WebRTC data channels; `node/test/ws-rtc-multi-transport.test.mts` confirms both transports share one Room with no Room changes; task 093, 2026-06-09.
